...
pub fn show(ui: &mut Ui, state: &mut AppState) {
    ...
    let mut all: Vec<FlatHost> = state.networks.iter().flat_map(|net| {
        net.hosts.iter().map(|h| {
            ...
            for node in state.workflow.snarl.nodes() {
                match node {
                    WorkflowNode::HostNode { ip: nip, hostname: nh, os_info: nos, admin: na, shares, users, .. } => {
                        if *nip == fh.ip {
                            if fh.hostname.is_empty() && !nh.is_empty() { fh.hostname = nh.clone(); }
                            if fh.os_info.is_empty() && !nos.is_empty() { fh.os_info = nos.clone(); }
                            if *na { fh.admin = true; fh.status = HostStatus::Accessible; }
                            if fh.shares_count == 0 && !shares.is_empty() { fh.shares_count = shares.len(); }
                            if fh.users_count == 0 && !users.is_empty() { fh.users_count = users.len(); }
                        }
                    }
                    WorkflowNode::SharesNode { host_ip, shares, .. } => {
    ...
    for node in state.workflow.snarl.nodes() {
        if let WorkflowNode::HostNode { ip, hostname, os_info, admin, shares, users, .. } = node {
            if !all.iter().any(|h| h.ip == *ip) {
                all.push(FlatHost {
                    ...
                    admin: *admin,
                    status: if *admin { HostStatus::Accessible } else { HostStatus::Unknown },
                });
            }
        }
    }
    ...
    let filtered: Vec<usize> = (0..all.len()).filter(|&i| {
    ...
    let authed = all.iter().filter(|h| matches!(h.status, HostStatus::Accessible | HostStatus::Locked)).count();
    ...
    ui.horizontal(|ui| {
        ui.add_space(pad);
        ui.label(egui::RichText::new("🎯 Targets").size(18.0).strong().color(Color32::WHITE));
        ui.add_space(16.0);
    ...
    // Separator
    {
        ...
        ui.painter().hline((r.left() + pad)..=(r.right() - pad), r.top(), egui::Stroke::new(1.0, SEPARATOR));
        ui.add_space(2.0);
    }
    ...
    let c_users = tw * 0.08;
    let col_labels = [
        ("IP", c_ip), ("PROTOCOL", c_proto), ("STATUS", c_status),
        ("HOSTNAME", c_host), ("OS", c_os), ("SHARES", c_shares), ("USERS", c_users),
    ];

    // ── Header row ──
    {
        let (hr, _) = ui.allocate_exact_size(egui::vec2(ui.available_width(), 22.0), egui::Sense::hover());
        let y = hr.center().y;
        let mut x = hr.left() + pad;
        let hfont = egui::FontId::new(10.0, egui::FontFamily::Monospace);
        for (lbl, w) in &col_labels {
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, *lbl, hfont.clone(), TEXT_DIM);
            x += w;
        }
    }
    ...
    egui::ScrollArea::vertical().auto_shrink([false; 2]).show(ui, |ui| {
        ...
        for (row_idx, &host_idx) in filtered.iter().enumerate() {
            ...
            // Background
            let bg = if row_resp.hovered() { ROW_HOVER } else if is_alt { ROW_ALT } else { Color32::TRANSPARENT };
            ...
            // IP
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, &h.ip, fm.clone(), Color32::WHITE);
            x += c_ip;

            // Protocol
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, "SMB", fs.clone(), BLUE);
            x += c_proto;

            // Status badge
            if h.admin {
                let br = Rect::from_min_size(Pos2::new(x + 2.0, y - 9.0), egui::vec2(66.0, 18.0));
                ui.painter().rect_filled(br, egui::CornerRadius::same(3), Color32::from_rgb(150, 30, 30));
                ui.painter().text(br.center(), egui::Align2::CENTER_CENTER, "Pwn3d!", fs.clone(), Color32::WHITE);
            } else {
                match h.status {
                    HostStatus::Accessible | HostStatus::Locked => {
                        let br = Rect::from_min_size(Pos2::new(x + 2.0, y - 9.0), egui::vec2(66.0, 18.0));
                        ui.painter().rect_filled(br, egui::CornerRadius::same(3), Color32::from_rgb(25, 55, 35));
                        ui.painter().text(br.center(), egui::Align2::CENTER_CENTER, "Auth ✔", fs.clone(), GREEN);
                    }
                    HostStatus::Unknown => {
                        ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, "—", fs.clone(), TEXT_DIM);
                    }
                }
            }
            x += c_status;
            ...
            // Hostname
            let hn = if h.hostname.is_empty() { "—" } else { &h.hostname };
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, hn, fp.clone(), Color32::WHITE);
            x += c_host;

            // OS
            let os = if h.os_info.is_empty() { "—" } else { &h.os_info };
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, os, fps.clone(), TEXT_DIM);
            x += c_os;

            // Shares
            let sc = h.shares_count;
            let (st, sc_col) = if sc > 0 { (format!("{sc}"), BLUE) } else { ("—".into(), TEXT_DIM) };
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, &st, fs.clone(), sc_col);
            x += c_shares;

            // Users
            let uc = h.users_count;
            let (ut, uc_col) = if uc > 0 { (format!("{uc}"), BLUE) } else { ("—".into(), TEXT_DIM) };
            ui.painter().text(Pos2::new(x + 4.0, y), egui::Align2::LEFT_CENTER, &ut, fs.clone(), uc_col);
        ...
        if let Some(idx) = send_idx {
            ...
            let added = state.workflow.add_host_node(
                h.ip.clone(), h.hostname.clone(), h.os_info.clone(),
                Vec::new(), h.admin, Vec::new(),
            );
            if added {
        ...
        if filtered.is_empty() {
            ...
            ui.vertical_centered(|ui| {
                if total == 0 {
                    ui.label(egui::RichText::new("No targets scanned yet").size(16.0).color(TEXT_DIM));
                    ui.add_space(8.0);
                    ui.label(egui::RichText::new("Run a scan from the Workspace tab to populate this view").size(12.0).color(TEXT_DIM).italics());
                } else {
                    ui.label(egui::RichText::new("No hosts match your search").size(14.0).color(TEXT_DIM));
                }
            });
        }
    });
...
fn badge(ui: &mut Ui, value: &str, label: &str, color: Color32) {
    ...
    let galley = ui.painter().layout_no_wrap(text.clone(), egui::FontId::new(10.0, egui::FontFamily::Monospace), color);
    ...
    ui.painter().text(rect.center(), egui::Align2::CENTER_CENTER, &text, egui::FontId::new(10.0, egui::FontFamily::Monospace), color);
}
