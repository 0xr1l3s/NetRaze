...
mod tests {
    ...
    fn samr_connect2_request_ip_matches_impacket() {
        // Fixture generated by Impacket with an RFC 5737 documentation address.
        let stub = encode_samr_connect2_request(Some("192.0.2.10"), MAXIMUM_ALLOWED);
        assert_eq!(stub.len(), 40, "SamrConnect2 IP stub length mismatch");
        // Body after referent_id (skip first 4 bytes)
        let expected_body = [
            // max_count = 10
            0x0a, 0x00, 0x00, 0x00, // offset = 0
            0x00, 0x00, 0x00, 0x00, // actual_count = 10
            0x0a, 0x00, 0x00, 0x00, // "192.0.2.10" UTF-16LE
            0x31, 0x00, 0x39, 0x00, 0x32, 0x00, 0x2e, 0x00, 0x30, 0x00, 0x2e, 0x00, 0x32, 0x00,
            0x2e, 0x00, 0x31, 0x00, 0x30, 0x00, // DesiredAccess = 0x02000000
            0x00, 0x00, 0x00, 0x02,
        ];
        assert_eq!(
            &stub[4..],
