...
| File | Role |
|---|---|
| `docker-compose.yml` | Spins up `ghcr.io/servercontainers/samba:smbd-only-latest` on `127.0.0.1:1445` and requires the test password through `NETRAZE_SAMBA_PASSWORD`. |
| `smb.conf` | Pinned share inventory. Share names and `comment =` values are load-bearing — Rust tests assert on them. |

The container provisions a single user, `alice`, in the `NETRAZE` workgroup.
Its password is generated by the operator and is never stored in Git. Wrong
passwords map onto the guest account
...
the tests don't race the daemon's startup. Keep this environment variable in
the same shell when running Cargo; Compose and the Rust tests must receive the
same value.
...
### Compose reports that the password variable is unset

Generate `NETRAZE_SAMBA_PASSWORD` in the current shell before both `docker
compose` and `cargo test`. The harness deliberately has no repository default.
